...
public class AffineTransformation implements Transformation {
	...
	public enum AffineType{TRANSLATIONX, TRANSLATIONY, ROTATION, SCALINGX, SCALINGY, SHEARX, SHEARY};
	public enum AffineSign{POSITIVE, NEGATIVE};
	...
	/**
	 * @return Horizontal component of the transformation, a.k.a. c-factor of
	 *         the matrix.
	 */
	...
	/**
	 * @return Vertical component of the transformation, a.k.a. f-factor of the
	 *         matrix.
	 */
	...
	public static AffineTransformation scaleToRectangle(Rectangle source, Rectangle target){

		/*
		 * When we go from one rectangle to another, we must go through 3 different Transformations :
		 * 1 : move the center of source to the ORIGIN
		 * 2 : scale it to target's ratio
		 * 3 : move the center back to where it first was
		 */
		
		AffineTransformation transform1 = newTranslation(-source.center().x(), -source.center().y());
		AffineTransformation transform2 = newScaling(target.width()/source.width(), -target.height()/source.height());
		AffineTransformation transform3 = newTranslation(target.center().x(), target.center().y());
		
		return transform3.composeWith(transform2).composeWith(transform1);
	}
	
	/**
	 * Implementation of the Strategy pattern, takes a type of transformation and the sign
	 * and offers a method createsTransformationWithValue(double value) that returns an affine
	 * transformation with the parameters given and the value.
	 */
	public static class affineCreator{
		
		AffineType type;
		AffineSign sign;
		
		public affineCreator(AffineType type, AffineSign sign){
			this.type = type;
			this.sign = sign;
		}
		
		/**
		 * creates an affine transformation of the type of the affineCreator with the value
		 * given in argument
		 * @param value - value of the affine transformation.
		 * @return an affine transformation with the value given and the parameters of the instance.
		 */
		public AffineTransformation createTransformationWithValue(double value){
			
			// We first calculate the sign value from the enumeration given :
			int signFactor = (sign.equals(AffineSign.POSITIVE)) ? 1 : -1;
			// We initalize it to IDENTITY in case there is a bug in the switch :
			AffineTransformation transformation = AffineTransformation.IDENTITY;
			
			switch(type){
			case TRANSLATIONX :
				transformation = AffineTransformation.newTranslation(signFactor * value, 0);
				break;
			case TRANSLATIONY :
				transformation = AffineTransformation.newTranslation(0, signFactor * value);
				break;
			case ROTATION :
				transformation = AffineTransformation.newRotation(signFactor*value * Math.PI / 180.0);
				break;
			case SCALINGX :
				transformation = AffineTransformation.newScaling(Math.pow(value, signFactor), 1);
				break;
			case SCALINGY :
				transformation = AffineTransformation.newScaling(1, Math.pow(value, signFactor));
				break;
			case SHEARX :
				transformation = AffineTransformation.newShearX(signFactor * value);
				break;
			case SHEARY :
				transformation = AffineTransformation.newShearY(signFactor * value);
				break;
			}
